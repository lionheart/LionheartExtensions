...
func async(queue: dispatch_queue_t, delay: Double = 0, callback: Void -> Void) {
    if delay > 0 {
...
func async_main(delay delay: Double = 0, callback: Void -> Void) {
    async(dispatch_get_main_queue(), delay: delay) {
        callback()
    }
}

func async_default(delay delay: Double = 0, callback: Void -> Void) {
    async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), delay: delay) {
        callback()
    }
}

func async_background(delay delay: Double = 0, callback: Void -> Void) {
