...
extension UIImage {
  ...
  /**
     ...
     - Date: March 9, 2016
     */
  public convenience init?(base64DataURLString: String?) {
  ...
  /**
     ...
     - Date: February 17, 2016
     */
  public func image(withAlpha alpha: Float) -> UIImage? {
  ...
  public class func screenshot() -> UIImage? {
    ...
    for window in windows {
      ...
      guard
        let statusBarOrientation = LionheartExtensions.sharedUIApplication?.connectedScenes.filter({
          $0.activationState == .foregroundActive
        }).map({ $0 as? UIWindowScene }).compactMap({ $0 }).first?.interfaceOrientation
      else {
        continue
      }
  ...
  /**
     ...
     - Date: July 20, 2016
     */
  public func saveToFile(_ path: String, format: UIImageFormat) throws {
    let data: Data?
    switch format {
    case .PNG:
      data = pngData()
  ...
  /**
     ...
     */
  @available(iOS 9.0, *)
  public func saveToCameraRoll(_ completion: ((Bool, NSError?) -> Void)?) throws {
