...
public enum ColorRepresentation: ExpressibleByIntegerLiteral, ExpressibleByArrayLiteral,
  ExpressibleByStringLiteral
{
  ...
  public init(from string: String) {
    let hexColorRegularExpression = try! NSRegularExpression(
      pattern: "^#?([0-9A-Fa-f]{6}|[0-9A-Fa-f]{3})$", options: [])
    let rgbColorRegularExpression = try! NSRegularExpression(
      pattern:
        "^rgb\\((1?[0-9]{1,2}|2[0-5][0-5]),[ ]*(1?[0-9]{1,2}|2[0-5][0-5]),[ ]*(1?[0-9]{1,2}|2[0-5][0-5])\\)$",
      options: [])
    let rgbaColorRegularExpression = try! NSRegularExpression(
      pattern:
        "^rgba\\((1?[0-9]{1,2}|2[0-5][0-5]),[ ]*(1?[0-9]{1,2}|2[0-5][0-5]),[ ]*(1?[0-9]{1,2}|2[0-5][0-5]),[ ]*(0?\\.\\d+)\\)$",
      options: [])

    let stringValue = string as NSString
    if let match = hexColorRegularExpression.firstMatch(
      in: string, options: [], range: NSMakeRange(0, string.count))
    {
